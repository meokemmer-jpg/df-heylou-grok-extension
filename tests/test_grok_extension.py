"""Tests for GrokExtension dispatch [CRUX-MK]."""
...
from src.grok_extension import (
    ExtensionProvenance,
    ExtensionResponse,
    GrokExtension,
    K0_FUNCTIONS,
...
def test_unknown_function_returns_error():
    ext = GrokExtension(sandbox_mode=True)
...
def test_book_direct_is_k0_relevant_in_sandbox_safe():
    ...
    assert resp.data["booking_id"].startswith("MOCK-grok-")
...
def test_real_mode_requires_phronesis_ticket():
    ...
    assert ext.is_real_mode() is False
    os.environ.pop("DF_HEYLOU_GROK_EXT_ENABLED", None)
...
def test_provenance_envelope_has_required_fields():
    ...
    assert p.extension_id == "df-heylou-grok-extension"
    assert p.provider == "grok"
