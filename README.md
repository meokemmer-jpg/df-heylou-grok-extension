# df-heylou-grok-extension [CRUX-MK]

Welle-39 LLM Sub-Funktion Extension: **HeyLou als Sub-Funktion in Grok Function-Calling**.

Stack: xAI Grok MCP-Server / Function-Calling.
...
Grok Function-Calling: Tool-Declaration als JSON-Schema, Modell ruft `functionCall` → unsere Extension routet zu HeyLou-API-Backend.

```python
tools = [{"function_declarations": HEYLOU_FUNCTION_DEFINITIONS}]
response = grok_client.generate_content(prompt, tools=tools)
...
- `DF_HEYLOU_GROK_EXT_ENABLED=false` → Mock-Responses (HeyLou Travel-Knowledge-Graph synthetisch)
- `DF_HEYLOU_GROK_EXT_ENABLED=true` + `PHRONESIS_TICKET` + `GROK_API_KEY` → Real-Mode

## Architektur

```
Grok-LLM → functionCall → GrokExtension.handle_function_call()
...
- Plist: `scripts/com.kemmer.df-heylou-grok-extension.plist`
- StartInterval: 7200s (2h), RunAtLoad: true
- WorkingDir: `/Users/make/Projects/dark-factories/df-heylou-grok-extension`
