...
Phasen:
1. auth   -> GrokAuthHandler.get_credentials()
...
DF_ID = "df-heylou-grok-extension"
PROVIDER = "gemini"
...
class GrokAdapterOrchestrator:
    DF_ID = DF_ID
    PROVIDER = PROVIDER

    def __init__(self, tenant_id: Optional[str] = None):
        self.tenant_id = tenant_id or os.environ.get(
            "DF_HEYLOU_GROK_TENANT_ID", "hildesheim"
        )
        self.sandbox_mode = (
            os.environ.get("DF_HEYLOU_GROK_EXT_ENABLED", "false") != "true"
        )
        from src.grok_extension import GrokExtension
        from src.auth_handler import GrokAuthHandler
        from src.audit_logger import AuditLogger
        from src.function_definitions import get_function_names

        self.extension = GrokExtension(sandbox_mode=self.sandbox_mode)
        self.auth = GrokAuthHandler(sandbox_mode=self.sandbox_mode)
