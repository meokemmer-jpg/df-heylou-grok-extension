"""Grok-Auth-Handler fuer HeyLou-Extension [CRUX-MK].

Welle-39. User-OAuth (GROK_API_KEY) + Service-Identity-Token (DF_HEYLOU_GROK_HMAC_SECRET).
...
class GrokAuthHandler:
    """Gemini-Auth-Handler.

    Public-API:
    - get_credentials(tenant_id) -> GrokCredentials
    - validate(creds) -> bool
    ...
    def __init__(self, sandbox_mode: bool | None = None):
        if sandbox_mode is None:
            sandbox_mode = os.environ.get("DF_HEYLOU_GROK_EXT_ENABLED", "false") != "true"
        self.sandbox_mode = sandbox_mode

    def _now_iso(self) -> str:
        return datetime.now(timezone.utc).isoformat()

    def get_credentials(self, tenant_id: str = "hildesheim") -> GrokCredentials:
        """Fetch credentials from ENV + tenant_id."""
        api_key = os.environ.get("GROK_API_KEY", "")
        phronesis = (
            os.environ.get("PHRONESIS_TICKET")
            or os.environ.get("DF_HEYLOU_GROK_PHRONESIS_TICKET")
        )

        if self.sandbox_mode:
            return GrokCredentials(
                api_key=api_key or "sandbox-fake-grok-key",
    ...
    def validate(self, creds: GrokCredentials) -> bool:
